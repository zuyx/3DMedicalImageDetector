...
class Net(nn.Module):
    def __init__(self,anchor_num):
        ...
        self.nodule_output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                           nn.ReLU(),
                                           # nn.Dropout3d(p = 0.3),
                                           nn.Conv3d(64, self.anchor_num, kernel_size=1))

        self.regress_output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                            nn.ReLU(),
                                            # nn.Dropout3d(p = 0.3),
                                            nn.Conv3d(64, 4 * self.anchor_num, kernel_size=1))
