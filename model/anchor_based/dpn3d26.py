...
class DPN(nn.Module):
    def __init__(self, cfg,anchor_num=3):
        ...
        self.output = nn.Sequential(nn.Conv3d(248, 64, kernel_size = 1),
                                    nn.ReLU(),
                                    #nn.Dropout3d(p = 0.3),
                                   nn.Conv3d(64, 5 * self.anchor_num, kernel_size = 1))
    ...
    def forward(self, x, coord):
        ...
        out = out.transpose(1, 2).contiguous().view(size[0], size[2], size[3], size[4], self.anchor_num, 5)
        if debug: print ('12', out.size())
        return out#, out_1

def DPN92_3D(anchor_num):
    cfg = {
    ...
    return DPN(cfg,anchor_num=anchor_num)

def get_model(config):
    net = DPN92_3D(len(config['anchors']))
    print('Model----dpn3d26!')
    return net
