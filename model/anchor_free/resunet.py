...
class Net(nn.Module):
    def __init__(self):
        ...
        self.hmap_output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                           nn.ReLU(),
                                           # nn.Dropout3d(p = 0.3),
                                           nn.Conv3d(64, 1, kernel_size=1))
        self.diam_output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                            nn.ReLU(),
                                            # nn.Dropout3d(p = 0.3),
                                            nn.Conv3d(64, 1, kernel_size=1))
        self.offset_output = nn.Sequential(nn.Conv3d(self.featureNum_back[0], 64, kernel_size=1),
                                            nn.ReLU(),
                                            # nn.Dropout3d(p = 0.3),
                                            nn.Conv3d(64, 3, kernel_size=1))
        focal_bias = -math.log((1.0 - 0.01) / 0.01)
        self._modules['hmap_output'][2].bias.data.fill_(focal_bias)
    ...
    def forward(self, x, coord):
        ...
        fmap = comb2
        hmap = self.hmap_output(comb2)
        offset = self.offset_output(comb2)
        diam = self.diam_output(comb2)
        return fmap,hmap,offset,diam
