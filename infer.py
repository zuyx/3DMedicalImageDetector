...
nohup python infer.py  --save-dir $  --epochs $  > infer.out &

for example:
    nohup python infer.py --save-dir 'results/res18_old/config0'   --epochs 120  --gpu '0' --n_test 2 > infer.out &
'''
...
parser.add_argument('--resume', default='220.ckpt', type=str, metavar='PATH',
...
def test(epoch,data_loader, net, get_pbb, save_dir, config):
    ...
    with torch.no_grad():
        '''
            ...
            that is the shape of one split is (side_len,side_len,side_len)
        ...
        for i_name, (data, target, coord, nzhw) in enumerate(data_loader):
            ...
            for i in range(len(splitlist) - 1):
                ...
                output = output_collect(hmap,offset,diam)
                output_list.append(output)

            output = torch.cat(output_list,dim=0).cpu().numpy() # （output[0].size = (d,h,w,c)）
            output = split_comber.combine(output,nzhw=nzhw)
            pbb = get_pbb(output,threshold = thresh)
...
def main():
    ...
    test_patient_ids = np.load(prep_config['test_patient_ids'])
