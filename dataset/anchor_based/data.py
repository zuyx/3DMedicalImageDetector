...
from dataset.anchor_based.labelMapping import LabelMapping
...
class DataBowl3Detector(Dataset):
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            ...
            label = self.label_mapping(sample.shape[1:], target, bboxes, filename) # label数据
            # 归一化
            sample = (sample.astype(np.float32)-128)/128 # 图像数据
